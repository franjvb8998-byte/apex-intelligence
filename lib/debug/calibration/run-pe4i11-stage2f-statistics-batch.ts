/**
 * PE-4I.11 — Stage-2F statistics batch CLI (max 150 new provider calls).
 *
 *   npx tsx lib/debug/calibration/run-pe4i11-stage2f-statistics-batch.ts \
...
import {
  PE4I11_EXPECTED_INITIAL_CACHE_HITS,
  PE4I11_EXPECTED_INITIAL_UNCACHED,
  pe4I11ArtifactDir,
  runPe4I11Stage2FStatisticsBatch,
} from "@/lib/debug/calibration/pe4-acquisition/stage2f-batch";
import {
...
async function main(): Promise<void> {
  ...
  const preflight = {
    phase: "PE4I11_STAGE2F_PREFLIGHT",
    queueDigest: queue.queueDigest,
  ...
  if (preview.initialCachedCount !== PE4I11_EXPECTED_INITIAL_CACHE_HITS) {
    throw new Error(
      `STOP: expected exactly ${PE4I11_EXPECTED_INITIAL_CACHE_HITS} cache hits before Stage-2F; got ${preview.initialCachedCount}`,
    );
  }
  if (preview.initialUncachedCount !== PE4I11_EXPECTED_INITIAL_UNCACHED) {
    throw new Error(
      `STOP: expected exactly ${PE4I11_EXPECTED_INITIAL_UNCACHED} uncached before Stage-2F; got ${preview.initialUncachedCount}`,
    );
  }
  ...
  const report = await runPe4I11Stage2FStatisticsBatch({
    transport,
    maxCalls,
  });

  const reportPath = path.join(pe4I11ArtifactDir(), "stage2f-report.json");
  writeFileSync(reportPath, JSON.stringify(report, null, 2) + "\n", "utf8");
  console.log(JSON.stringify(report, null, 2));
  console.log(
    JSON.stringify(
      {
        stage2fReportPath: path
          .relative(process.cwd(), reportPath)
          .replace(/\\/g, "/"),
        artifactDir: path
          .relative(process.cwd(), pe4I11ArtifactDir())
          .replace(/\\/g, "/"),
      },
      null,
      2,
...
const invoked =
  process.argv[1]
    ?.replace(/\\/g, "/")
    .endsWith("run-pe4i11-stage2f-statistics-batch.ts") === true;
