/**
 * PE-4I.9 — Stage-2D statistics batch CLI (max 150 new provider calls).
 *
 *   npx tsx lib/debug/calibration/run-pe4i9-stage2d-statistics-batch.ts \
...
import {
  PE4I9_EXPECTED_INITIAL_CACHE_HITS,
  PE4I9_EXPECTED_INITIAL_UNCACHED,
  pe4I9ArtifactDir,
  runPe4I9Stage2DStatisticsBatch,
} from "@/lib/debug/calibration/pe4-acquisition/stage2d-batch";
import {
...
async function main(): Promise<void> {
  ...
  if (preview.initialCachedCount !== PE4I9_EXPECTED_INITIAL_CACHE_HITS) {
    throw new Error(
      `STOP: expected exactly ${PE4I9_EXPECTED_INITIAL_CACHE_HITS} cache hits before Stage-2D; got ${preview.initialCachedCount}`,
    );
  }
  if (preview.initialUncachedCount !== PE4I9_EXPECTED_INITIAL_UNCACHED) {
    throw new Error(
      `STOP: expected exactly ${PE4I9_EXPECTED_INITIAL_UNCACHED} uncached before Stage-2D; got ${preview.initialUncachedCount}`,
    );
  }
  ...
  const report = await runPe4I9Stage2DStatisticsBatch({
    transport,
    maxCalls,
  });

  const reportPath = path.join(pe4I9ArtifactDir(), "stage2d-report.json");
  writeFileSync(reportPath, JSON.stringify(report, null, 2) + "\n", "utf8");
  console.log(JSON.stringify(report, null, 2));
  console.log(
    JSON.stringify(
      {
        stage2dReportPath: path
          .relative(process.cwd(), reportPath)
          .replace(/\\/g, "/"),
        artifactDir: path
          .relative(process.cwd(), pe4I9ArtifactDir())
          .replace(/\\/g, "/"),
      },
      null,
      2,
...
const invoked =
  process.argv[1]
    ?.replace(/\\/g, "/")
    .endsWith("run-pe4i9-stage2d-statistics-batch.ts") === true;
