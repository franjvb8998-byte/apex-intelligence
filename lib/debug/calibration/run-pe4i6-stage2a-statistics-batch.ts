/**
 * PE-4I.6 — Stage-2A statistics batch CLI (max 150 new provider calls).
 *
 *   npx tsx lib/debug/calibration/run-pe4i6-stage2a-statistics-batch.ts \
...
} from "@/lib/debug/calibration/pe4-acquisition/stage2-auth";
import {
  pe4I6ArtifactDir,
  runPe4I6Stage2AStatisticsBatch,
} from "@/lib/debug/calibration/pe4-acquisition/stage2a-batch";
import {
...
async function main(): Promise<void> {
  ...
  const report = await runPe4I6Stage2AStatisticsBatch({
    transport,
    maxCalls,
  });

  const reportPath = path.join(pe4I6ArtifactDir(), "stage2a-report.json");
  writeFileSync(reportPath, JSON.stringify(report, null, 2) + "\n", "utf8");
  console.log(JSON.stringify(report, null, 2));
  console.log(
    JSON.stringify(
      {
        stage2aReportPath: path
          .relative(process.cwd(), reportPath)
          .replace(/\\/g, "/"),
        artifactDir: path
          .relative(process.cwd(), pe4I6ArtifactDir())
          .replace(/\\/g, "/"),
      },
      null,
      2,
...
const invoked =
  process.argv[1]
    ?.replace(/\\/g, "/")
    .endsWith("run-pe4i6-stage2a-statistics-batch.ts") === true;
