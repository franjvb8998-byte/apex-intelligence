/**
 * PE-4I.8 — Stage-2C statistics batch CLI (max 150 new provider calls).
 *
 *   npx tsx lib/debug/calibration/run-pe4i8-stage2c-statistics-batch.ts \
...
import {
  pe4I8ArtifactDir,
  runPe4I8Stage2CStatisticsBatch,
} from "@/lib/debug/calibration/pe4-acquisition/stage2c-batch";
import {
...
async function main(): Promise<void> {
  ...
  if (preview.initialCachedCount !== 301) {
    throw new Error(
      `STOP: expected exactly 301 cache hits before Stage-2C; got ${preview.initialCachedCount}`,
    );
  }
  if (preview.initialUncachedCount !== 793) {
    throw new Error(
      `STOP: expected exactly 793 uncached before Stage-2C; got ${preview.initialUncachedCount}`,
    );
  }
  ...
  const report = await runPe4I8Stage2CStatisticsBatch({
    transport,
    maxCalls,
  });

  const reportPath = path.join(pe4I8ArtifactDir(), "stage2c-report.json");
  writeFileSync(reportPath, JSON.stringify(report, null, 2) + "\n", "utf8");
  console.log(JSON.stringify(report, null, 2));
  console.log(
    JSON.stringify(
      {
        stage2cReportPath: path
          .relative(process.cwd(), reportPath)
          .replace(/\\/g, "/"),
        artifactDir: path
          .relative(process.cwd(), pe4I8ArtifactDir())
          .replace(/\\/g, "/"),
      },
      null,
      2,
...
const invoked =
  process.argv[1]
    ?.replace(/\\/g, "/")
    .endsWith("run-pe4i8-stage2c-statistics-batch.ts") === true;
