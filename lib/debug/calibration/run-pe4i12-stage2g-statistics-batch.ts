/**
 * PE-4I.12 — Stage-2G statistics batch CLI (max 150 new provider calls).
 *
 *   npx tsx lib/debug/calibration/run-pe4i12-stage2g-statistics-batch.ts \
...
import {
  PE4I12_EXPECTED_INITIAL_CACHE_HITS,
  PE4I12_EXPECTED_INITIAL_UNCACHED,
  pe4I12ArtifactDir,
  runPe4I12Stage2GStatisticsBatch,
} from "@/lib/debug/calibration/pe4-acquisition/stage2g-batch";
import {
...
async function main(): Promise<void> {
  ...
  const preflight = {
    phase: "PE4I12_STAGE2G_PREFLIGHT",
    queueDigest: queue.queueDigest,
  ...
  if (preview.initialCachedCount !== PE4I12_EXPECTED_INITIAL_CACHE_HITS) {
    throw new Error(
      `STOP: expected exactly ${PE4I12_EXPECTED_INITIAL_CACHE_HITS} cache hits before Stage-2G; got ${preview.initialCachedCount}`,
    );
  }
  if (preview.initialUncachedCount !== PE4I12_EXPECTED_INITIAL_UNCACHED) {
    throw new Error(
      `STOP: expected exactly ${PE4I12_EXPECTED_INITIAL_UNCACHED} uncached before Stage-2G; got ${preview.initialUncachedCount}`,
    );
  }
  ...
  const report = await runPe4I12Stage2GStatisticsBatch({
    transport,
    maxCalls,
  });

  const reportPath = path.join(pe4I12ArtifactDir(), "stage2g-report.json");
  writeFileSync(reportPath, JSON.stringify(report, null, 2) + "\n", "utf8");
  console.log(JSON.stringify(report, null, 2));
  console.log(
    JSON.stringify(
      {
        stage2gReportPath: path
          .relative(process.cwd(), reportPath)
          .replace(/\\/g, "/"),
        artifactDir: path
          .relative(process.cwd(), pe4I12ArtifactDir())
          .replace(/\\/g, "/"),
      },
      null,
      2,
...
const invoked =
  process.argv[1]
    ?.replace(/\\/g, "/")
    .endsWith("run-pe4i12-stage2g-statistics-batch.ts") === true;
