/**
 * PE-4I.10 — Stage-2E statistics batch CLI (max 150 new provider calls).
 *
 *   npx tsx lib/debug/calibration/run-pe4i10-stage2e-statistics-batch.ts \
...
import {
  PE4I10_EXPECTED_INITIAL_CACHE_HITS,
  PE4I10_EXPECTED_INITIAL_UNCACHED,
  pe4I10ArtifactDir,
  runPe4I10Stage2EStatisticsBatch,
} from "@/lib/debug/calibration/pe4-acquisition/stage2e-batch";
import {
...
async function main(): Promise<void> {
  ...
  const preflight = {
    phase: "PE4I10_STAGE2E_PREFLIGHT",
    queueDigest: queue.queueDigest,
  ...
  if (preview.initialCachedCount !== PE4I10_EXPECTED_INITIAL_CACHE_HITS) {
    throw new Error(
      `STOP: expected exactly ${PE4I10_EXPECTED_INITIAL_CACHE_HITS} cache hits before Stage-2E; got ${preview.initialCachedCount}`,
    );
  }
  if (preview.initialUncachedCount !== PE4I10_EXPECTED_INITIAL_UNCACHED) {
    throw new Error(
      `STOP: expected exactly ${PE4I10_EXPECTED_INITIAL_UNCACHED} uncached before Stage-2E; got ${preview.initialUncachedCount}`,
    );
  }
  ...
  const report = await runPe4I10Stage2EStatisticsBatch({
    transport,
    maxCalls,
  });

  const reportPath = path.join(pe4I10ArtifactDir(), "stage2e-report.json");
  writeFileSync(reportPath, JSON.stringify(report, null, 2) + "\n", "utf8");
  console.log(JSON.stringify(report, null, 2));
  console.log(
    JSON.stringify(
      {
        stage2eReportPath: path
          .relative(process.cwd(), reportPath)
          .replace(/\\/g, "/"),
        artifactDir: path
          .relative(process.cwd(), pe4I10ArtifactDir())
          .replace(/\\/g, "/"),
      },
      null,
      2,
...
const invoked =
  process.argv[1]
    ?.replace(/\\/g, "/")
    .endsWith("run-pe4i10-stage2e-statistics-batch.ts") === true;
