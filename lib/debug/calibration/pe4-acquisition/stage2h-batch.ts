/**
 * PE-4I.13 — Stage-2H FINAL bounded statistics acquisition (max 43).
 * Same frozen queue / auth. Stage-2H hard max 43 (auth ceiling remains 150).
 * Keeps competition coverage matrix (observed-sample xG labels only — not model
 * branches), provider season 2024 cumulative audit, calendar-2025 batch + FINAL
 * cumulative audits, and full-queue resume / completion audit. No modeling.
 */
...
export const PE4I13_STAGE2H_VERSION =
  "pe4.stage2h_final_statistics_acquisition.v1" as const;

export const PE4I13_ARTIFACT_SUBDIR = "stage2h-i13" as const;

/** Authoritative post-PE-4I.12 cache state before Stage-2H. */
export const PE4I13_EXPECTED_INITIAL_CACHE_HITS = 1051 as const;
export const PE4I13_EXPECTED_INITIAL_UNCACHED = 43 as const;

/** Stage-2H final-batch hard ceiling (auth Stage-2A ceiling remains 150). */
export const PE4I13_STAGE2H_HARD_MAX_CALLS = 43 as const;

/** Coverage labels only — NOT model branches / capability gates. */
export type Pe4I13ObservedXgLabel =
  | "XG_CAPABLE_IN_OBSERVED_SAMPLE"
  | "XG_PARTIAL_IN_OBSERVED_SAMPLE"
  | "XG_UNAVAILABLE_IN_OBSERVED_SAMPLE";

export function classifyPe4I13ObservedXgLabel(
  bothShare: number,
): Pe4I13ObservedXgLabel {
...
function artifactRoot(cwd: string): string {
  return path.join(cwd, PE4I2_CACHE_ROOT_RELATIVE, PE4I13_ARTIFACT_SUBDIR);
}
...
export type Pe4I13AetPenAuditRow = {
  providerFixtureId: string;
  status: string;
  kickoffUtc: string;
...
export type Pe4I13StatusAuditRow = {
  providerFixtureId: string;
  status: string;
  competition: string | null;
...
export type Pe4I13CompetitionFieldCoverageRow = {
  competition: string;
  fixtureCount: number;
...
export type Pe4I13SeasonCoverageRow = {
...
export type Pe4I13CompetitionCoverageMatrixRow = {
  competition: string;
  fixtureCount: number;
  xgBothTeamFixtures: number;
  xgOneTeamFixtures: number;
...
function buildCompetitionFieldCoverage(
  ...
  envelopes: readonly Pe4I2RawStatisticsEnvelope[],
  metaMap: ReadonlyMap<string, Pe4I6FixtureMeta>,
): Pe4I13CompetitionFieldCoverageRow {
...
function buildCompetitionCoverageMatrixRow(
  ...
  envelopes: readonly Pe4I2RawStatisticsEnvelope[],
  metaMap: ReadonlyMap<string, Pe4I6FixtureMeta>,
): Pe4I13CompetitionCoverageMatrixRow {
  const xgLocal = measureXgCoverage({
    envelopes,
    metaByFixtureId: metaMap,
  });
  ...
  return {
    ...
    yellowCardsCoveragePercent: fieldPresentShare(envelopes, "yellow_cards"),
    redCardsCoveragePercent: fieldPresentShare(envelopes, "red_cards"),
    observedXgLabel: classifyPe4I13ObservedXgLabel(bothShare),
    observedXgLabelNote:
...
function buildProviderSeason2024Coverage(
  ...
  envelopes: readonly Pe4I2RawStatisticsEnvelope[],
  metaMap: ReadonlyMap<string, Pe4I6FixtureMeta>,
): Pe4I13ProviderSeason2024Coverage | null {
...
export type Pe4I13Stage2HReport = {
  phase: "PE4I13_STAGE2H_FINAL_STATISTICS_ACQUISITION";
  stage2hVersion: typeof PE4I13_STAGE2H_VERSION;
  queueDigest: string;
  queueSize: number;
  ...
  finalQueueCompletionAudit: {
    queueSize: number;
    validStatisticsCacheHits: number;
    remainingUncachedFixtures: number;
    successfulQueueCoveragePercent: number;
    complete: boolean;
  };
  ...
  resumeVerification: {
    ...
    fullQueue: {
      providerCalls: number;
      fullQueueCacheHits: number;
      fullQueueUncached: number;
      fullQueueFailuresOrConflicts: number;
    };
  };
  ...
  competitionSpecificCoverage: Pe4I13CompetitionFieldCoverageRow[];
  cumulativeCompetitionCoverage: Pe4I13CompetitionFieldCoverageRow[];
  competitionCoverageMatrix: Pe4I13CompetitionCoverageMatrixRow[];
  competitionCoverageMatrixNote: string;
  providerSeasonCoverage: Pe4I13SeasonCoverageRow[];
  providerSeason2024TransitionReached: boolean;
  providerSeason2024Coverage: Pe4I13ProviderSeason2024Coverage | null;
  ...
  statusAudit: Pe4I13StatusAuditRow[];
  aetPenAudit: Pe4I13AetPenAuditRow[];
  calendar2025FixturesInBatch: number;
  calendar2025FixtureIds: string[];
  /** All calendarKickoffYear===2025 fixtures among successfully cached Stage-2 queue envelopes. */
  cumulativeCalendar2025RawFixtures: number;
  cumulativeCalendar2025FixtureIds: string[];
  cumulativeCalendar2025Note: string;
...
export async function runPe4I13Stage2HFinalStatisticsBatch(input: {
  transport: Pe4I2ProviderTransport;
  maxCalls: number;
  cwd?: string;
  acquiredAtUtc?: string;
}): Promise<Pe4I13Stage2HReport> {
  ...
  if (input.maxCalls > PE4I13_STAGE2H_HARD_MAX_CALLS) {
    throw new Error(
      `maxCalls ${input.maxCalls} exceeds Stage-2H hard max ${PE4I13_STAGE2H_HARD_MAX_CALLS}`,
    );
  }
  ...
  if (baseSelection.initialCachedCount !== PE4I13_EXPECTED_INITIAL_CACHE_HITS) {
    throw new Error(
      `STOP: expected exactly ${PE4I13_EXPECTED_INITIAL_CACHE_HITS} statistics cache hits before Stage-2H; got ${baseSelection.initialCachedCount}`,
    );
  }
  if (baseSelection.initialUncachedCount !== PE4I13_EXPECTED_INITIAL_UNCACHED) {
    throw new Error(
      `STOP: expected exactly ${PE4I13_EXPECTED_INITIAL_UNCACHED} uncached before Stage-2H; got ${baseSelection.initialUncachedCount}`,
    );
  }
  if (
    baseSelection.initialUncachedCount <= PE4I13_STAGE2H_HARD_MAX_CALLS &&
    baseSelection.selectedBatchSize !== baseSelection.initialUncachedCount
  ) {
    throw new Error(
      `STOP: selectedBatchSize ${baseSelection.selectedBatchSize} must equal initialUncachedCount ${baseSelection.initialUncachedCount} when uncached <= ${PE4I13_STAGE2H_HARD_MAX_CALLS}`,
    );
  }
  ...
  const unitResults: Pe4I13Stage2HReport["firstPass"]["unitResults"] = [];
  let newWrites = 0;
  ...
  const competitionCoverageMatrix: Pe4I13CompetitionCoverageMatrixRow[] = (() => {
    const byComp = new Map<
      string,
      { ids: string[]; envelopes: Pe4I2RawStatisticsEnvelope[] }
    >();
  ...
  const providerSeasonCoverage: Pe4I13SeasonCoverageRow[] = [...bySeason.entries()]
    .map(([season, b]) => {
      const xgLocal = measureXgCoverage({
  ...
  const statusAudit: Pe4I13StatusAuditRow[] = [];
  const aetPenAudit: Pe4I13AetPenAuditRow[] = [];
  ...
  const cumulativeCalendar2025FixtureIds = cumulativeCachedIds.filter((id) => {
    const meta = metaMap.get(id);
    return meta?.calendarKickoffYear === 2025;
  });
  const cumulativeCalendar2025Note =
    "CUMULATIVE_CALENDAR_2025_RAW_FIXTURES: raw-only under sealed holdout — no fitting, selection, calibration, or evaluation";
  ...
  const validStatisticsCacheHits = queue.fixtureIds.length - remainingUncached;
  const successfulQueueCoveragePercent =
    queue.fixtureIds.length === 0
      ? 0
      : Math.round(
          (validStatisticsCacheHits / queue.fixtureIds.length) * 100000,
        ) / 1000;
  const finalQueueCompletionAudit = {
    queueSize: queue.fixtureIds.length,
    validStatisticsCacheHits,
    remainingUncachedFixtures: remainingUncached,
    successfulQueueCoveragePercent,
    complete:
      remainingUncached === 0 &&
      validStatisticsCacheHits === queue.fixtureIds.length,
  };
  const acquisitionComplete = finalQueueCompletionAudit.complete;
  ...
  let fullQueueCacheHits = 0;
  let fullQueueUncached = 0;
  let fullQueueFailuresOrConflicts = 0;
  for (const id of queue.fixtureIds) {
    const key = statisticsUnitKey(id);
    let completedHit = false;
    let nonCompletedPresent = false;
    for (const cache of cachesAfter) {
      const unit = cache.getUnit(key);
      if (!unit) continue;
    ...
    if (completedHit) {
      const env = readStatisticsEnvelope(id, cachesAfter);
      if (env) fullQueueCacheHits += 1;
      else {
        fullQueueFailuresOrConflicts += 1;
        errors.push(
          `full-queue resume: completed cache missing envelope ${id}`,
        );
      }
    } else if (nonCompletedPresent) {
      fullQueueFailuresOrConflicts += 1;
    } else {
      fullQueueUncached += 1;
    }
  ...
  put("batch-manifest.json", {
    stage2hVersion: PE4I13_STAGE2H_VERSION,
    acquiredAtUtc,
  ...
  put("calendar-2025-cumulative-audit.json", {
    count: cumulativeCalendar2025FixtureIds.length,
    fixtureIds: cumulativeCalendar2025FixtureIds,
    note: cumulativeCalendar2025Note,
    finalCumulative: true,
  });
  put("final-queue-completion-audit.json", finalQueueCompletionAudit);
  put("call-accounting.json", {
    configuredMaxCalls: input.maxCalls,
    hardMaxCalls: PE4I13_STAGE2H_HARD_MAX_CALLS,
    queueSize: queue.fixtureIds.length,
  ...
  put("resume-verification.json", resumeVerification);
  put("final-resume-verification.json", resumeVerification);
  ...
  return {
    ...
    cumulativeCalendar2025RawFixtures: cumulativeCalendar2025FixtureIds.length,
    cumulativeCalendar2025FixtureIds,
    cumulativeCalendar2025Note,
    holdout2025Status: "SEALED",
...
export function pe4I13ArtifactDir(cwd = process.cwd()): string {
