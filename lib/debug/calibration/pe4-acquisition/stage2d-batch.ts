 ...
 * Same frozen queue / auth / hard max 150. Adds status audit + competition-specific
 * field coverage + cumulative competition coverage. No modeling.
 */
...
export const PE4I9_STAGE2D_VERSION =
  "pe4.stage2d_statistics_batch.v1" as const;

export const PE4I9_ARTIFACT_SUBDIR = "stage2d-i9" as const;

/** Authoritative post-PE-4I.8 cache state before Stage-2D. */
export const PE4I9_EXPECTED_INITIAL_CACHE_HITS = 451 as const;
export const PE4I9_EXPECTED_INITIAL_UNCACHED = 643 as const;
...
function artifactRoot(cwd: string): string {
  return path.join(cwd, PE4I2_CACHE_ROOT_RELATIVE, PE4I9_ARTIFACT_SUBDIR);
}
...
export type Pe4I9AetPenAuditRow = {
  providerFixtureId: string;
  status: string;
  kickoffUtc: string;
...
export type Pe4I9StatusAuditRow = {
  providerFixtureId: string;
  status: string;
  competition: string | null;
...
export type Pe4I9CompetitionFieldCoverageRow = {
  competition: string;
  fixtureCount: number;
...
export type Pe4I9SeasonCoverageRow = {
...
function buildCompetitionFieldCoverage(
  ...
  envelopes: readonly Pe4I2RawStatisticsEnvelope[],
  metaMap: ReadonlyMap<string, Pe4I6FixtureMeta>,
): Pe4I9CompetitionFieldCoverageRow {
...
export type Pe4I9Stage2DReport = {
  phase: "PE4I9_STAGE2D_STATISTICS_BATCH";
  stage2dVersion: typeof PE4I9_STAGE2D_VERSION;
  queueDigest: string;
  queueSize: number;
  ...
  competitionSpecificCoverage: Pe4I9CompetitionFieldCoverageRow[];
  cumulativeCompetitionCoverage: Pe4I9CompetitionFieldCoverageRow[];
  providerSeasonCoverage: Pe4I9SeasonCoverageRow[];
  providerSeason2024TransitionReached: boolean;
  providerSeason2024Coverage: Pe4I9SeasonCoverageRow | null;
  ...
  statusAudit: Pe4I9StatusAuditRow[];
  aetPenAudit: Pe4I9AetPenAuditRow[];
...
export async function runPe4I9Stage2DStatisticsBatch(input: {
  transport: Pe4I2ProviderTransport;
  maxCalls: number;
  cwd?: string;
  acquiredAtUtc?: string;
}): Promise<Pe4I9Stage2DReport> {
  ...
  if (baseSelection.initialCachedCount !== PE4I9_EXPECTED_INITIAL_CACHE_HITS) {
    throw new Error(
      `STOP: expected exactly ${PE4I9_EXPECTED_INITIAL_CACHE_HITS} statistics cache hits before Stage-2D; got ${baseSelection.initialCachedCount}`,
    );
  }
  if (baseSelection.initialUncachedCount !== PE4I9_EXPECTED_INITIAL_UNCACHED) {
    throw new Error(
      `STOP: expected exactly ${PE4I9_EXPECTED_INITIAL_UNCACHED} uncached before Stage-2D; got ${baseSelection.initialUncachedCount}`,
    );
  }
  ...
  const unitResults: Pe4I9Stage2DReport["firstPass"]["unitResults"] = [];
  let newWrites = 0;
  ...
  const providerSeasonCoverage: Pe4I9SeasonCoverageRow[] = [...bySeason.entries()]
    .map(([season, b]) => {
      const xgLocal = measureXgCoverage({
  ...
  const providerSeason2024Coverage =
    providerSeasonCoverage.find((r) => r.providerSeason === "2024") ?? null;

  const statusAudit: Pe4I9StatusAuditRow[] = [];
  const aetPenAudit: Pe4I9AetPenAuditRow[] = [];
  ...
  put("batch-manifest.json", {
    stage2dVersion: PE4I9_STAGE2D_VERSION,
    acquiredAtUtc,
...
export function pe4I9ArtifactDir(cwd = process.cwd()): string {
