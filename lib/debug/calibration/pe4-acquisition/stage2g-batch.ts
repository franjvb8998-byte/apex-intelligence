...
export const PE4I12_STAGE2G_VERSION =
  "pe4.stage2g_statistics_batch.v1" as const;

export const PE4I12_ARTIFACT_SUBDIR = "stage2g-i12" as const;

/** Authoritative post-PE-4I.11 cache state before Stage-2G. */
export const PE4I12_EXPECTED_INITIAL_CACHE_HITS = 901 as const;
export const PE4I12_EXPECTED_INITIAL_UNCACHED = 193 as const;

/** Coverage labels only — NOT model branches / capability gates. */
export type Pe4I12ObservedXgLabel =
  | "XG_CAPABLE_IN_OBSERVED_SAMPLE"
  | "XG_PARTIAL_IN_OBSERVED_SAMPLE"
  | "XG_UNAVAILABLE_IN_OBSERVED_SAMPLE";

export function classifyPe4I12ObservedXgLabel(
  bothShare: number,
): Pe4I12ObservedXgLabel {
...
function artifactRoot(cwd: string): string {
  return path.join(cwd, PE4I2_CACHE_ROOT_RELATIVE, PE4I12_ARTIFACT_SUBDIR);
}
...
export type Pe4I12AetPenAuditRow = {
  providerFixtureId: string;
  status: string;
  kickoffUtc: string;
...
export type Pe4I12StatusAuditRow = {
  providerFixtureId: string;
  status: string;
  competition: string | null;
...
export type Pe4I12CompetitionFieldCoverageRow = {
  competition: string;
  fixtureCount: number;
...
export type Pe4I12SeasonCoverageRow = {
...
export type Pe4I12CompetitionCoverageMatrixRow = {
  competition: string;
  fixtureCount: number;
  xgBothTeamFixtures: number;
  xgOneTeamFixtures: number;
...
function buildCompetitionFieldCoverage(
  ...
  envelopes: readonly Pe4I2RawStatisticsEnvelope[],
  metaMap: ReadonlyMap<string, Pe4I6FixtureMeta>,
): Pe4I12CompetitionFieldCoverageRow {
...
function buildCompetitionCoverageMatrixRow(
  ...
  envelopes: readonly Pe4I2RawStatisticsEnvelope[],
  metaMap: ReadonlyMap<string, Pe4I6FixtureMeta>,
): Pe4I12CompetitionCoverageMatrixRow {
  const xgLocal = measureXgCoverage({
    envelopes,
    metaByFixtureId: metaMap,
  });
  ...
  return {
    ...
    yellowCardsCoveragePercent: fieldPresentShare(envelopes, "yellow_cards"),
    redCardsCoveragePercent: fieldPresentShare(envelopes, "red_cards"),
    observedXgLabel: classifyPe4I12ObservedXgLabel(bothShare),
    observedXgLabelNote:
...
function buildProviderSeason2024Coverage(
  ...
  envelopes: readonly Pe4I2RawStatisticsEnvelope[],
  metaMap: ReadonlyMap<string, Pe4I6FixtureMeta>,
): Pe4I12ProviderSeason2024Coverage | null {
...
export type Pe4I12Stage2GReport = {
  phase: "PE4I12_STAGE2G_STATISTICS_BATCH";
  stage2gVersion: typeof PE4I12_STAGE2G_VERSION;
  queueDigest: string;
  queueSize: number;
  ...
  competitionSpecificCoverage: Pe4I12CompetitionFieldCoverageRow[];
  cumulativeCompetitionCoverage: Pe4I12CompetitionFieldCoverageRow[];
  competitionCoverageMatrix: Pe4I12CompetitionCoverageMatrixRow[];
  competitionCoverageMatrixNote: string;
  providerSeasonCoverage: Pe4I12SeasonCoverageRow[];
  providerSeason2024TransitionReached: boolean;
  providerSeason2024Coverage: Pe4I12ProviderSeason2024Coverage | null;
  ...
  statusAudit: Pe4I12StatusAuditRow[];
  aetPenAudit: Pe4I12AetPenAuditRow[];
...
export async function runPe4I12Stage2GStatisticsBatch(input: {
  transport: Pe4I2ProviderTransport;
  maxCalls: number;
  cwd?: string;
  acquiredAtUtc?: string;
}): Promise<Pe4I12Stage2GReport> {
  ...
  if (baseSelection.initialCachedCount !== PE4I12_EXPECTED_INITIAL_CACHE_HITS) {
    throw new Error(
      `STOP: expected exactly ${PE4I12_EXPECTED_INITIAL_CACHE_HITS} statistics cache hits before Stage-2G; got ${baseSelection.initialCachedCount}`,
    );
  }
  if (baseSelection.initialUncachedCount !== PE4I12_EXPECTED_INITIAL_UNCACHED) {
    throw new Error(
      `STOP: expected exactly ${PE4I12_EXPECTED_INITIAL_UNCACHED} uncached before Stage-2G; got ${baseSelection.initialUncachedCount}`,
    );
  }
  ...
  const unitResults: Pe4I12Stage2GReport["firstPass"]["unitResults"] = [];
  let newWrites = 0;
  ...
  const competitionCoverageMatrix: Pe4I12CompetitionCoverageMatrixRow[] = (() => {
    const byComp = new Map<
      string,
      { ids: string[]; envelopes: Pe4I2RawStatisticsEnvelope[] }
    >();
  ...
  const providerSeasonCoverage: Pe4I12SeasonCoverageRow[] = [...bySeason.entries()]
    .map(([season, b]) => {
      const xgLocal = measureXgCoverage({
  ...
  const statusAudit: Pe4I12StatusAuditRow[] = [];
  const aetPenAudit: Pe4I12AetPenAuditRow[] = [];
  ...
  put("batch-manifest.json", {
    stage2gVersion: PE4I12_STAGE2G_VERSION,
    acquiredAtUtc,
...
export function pe4I12ArtifactDir(cwd = process.cwd()): string {
