/**
 * PE-4I.7 — Stage-2C second bounded statistics batch.
 * Same frozen queue / auth / hard max 150. Adds calendar-year + cross-competition transition watch.
 * No modeling.
 */
...
export const PE4I8_STAGE2C_VERSION =
  "pe4.stage2c_statistics_batch.v1" as const;

export const PE4I8_ARTIFACT_SUBDIR = "stage2c-i8" as const;
...
function artifactRoot(cwd: string): string {
  return path.join(cwd, PE4I2_CACHE_ROOT_RELATIVE, PE4I8_ARTIFACT_SUBDIR);
}
...
export type Pe4I8AetPenAuditRow = {
  providerFixtureId: string;
  status: string;
  kickoffUtc: string;
...
export type Pe4I8CompetitionCoverageRow = {
  competition: string;
...
export type Pe4I8Stage2CReport = {
  phase: "PE4I8_STAGE2C_STATISTICS_BATCH";
  stage2cVersion: typeof PE4I8_STAGE2C_VERSION;
  queueDigest: string;
  queueSize: number;
  ...
  competitionCoverage: Pe4I8CompetitionCoverageRow[];
  providerSeasonCoverage: Pe4I8CompetitionCoverageRow[];
  ...
  aetPenAudit: Pe4I8AetPenAuditRow[];
...
export async function runPe4I8Stage2CStatisticsBatch(input: {
  transport: Pe4I2ProviderTransport;
  maxCalls: number;
  cwd?: string;
  acquiredAtUtc?: string;
}): Promise<Pe4I8Stage2CReport> {
  ...
  if (baseSelection.initialCachedCount < 301) {
    throw new Error(
      `STOP: expected >=301 prior statistics cache hits before Stage-2C; got ${baseSelection.initialCachedCount}`,
    );
  }
  if (baseSelection.initialUncachedCount !== 793) {
    // Allow drift only if extra cache appeared; refuse if more uncached than expected.
    if (baseSelection.initialUncachedCount > 793) {
      throw new Error(
        `STOP: uncached ${baseSelection.initialUncachedCount} > expected 793 — cache integrity suspect`,
      );
    }
  }
  ...
  const unitResults: Pe4I8Stage2CReport["firstPass"]["unitResults"] = [];
  let newWrites = 0;
  ...
  const competitionCoverage: Pe4I8CompetitionCoverageRow[] = [
    ...byComp.entries(),
  ]
    .map(([competition, b]) => {
  ...
  for (const id of selection.selectedFixtureIds) {
    ...
    bySeason.get(season)!.selected.push(id);
  }
  for (const u of unitResults) {
    const season = metaMap.get(u.fixtureId)?.providerSeason ?? "unknown";
  ...
  const providerSeasonCoverage: Pe4I8CompetitionCoverageRow[] = [
    ...bySeason.entries(),
  ]
    .map(([season, b]) => {
      const xgLocal = measureXgCoverage({
      ...
      return {
        competition: `providerSeason=${season}`,
        fixturesSelected: b.selected.length,
        callsAttempted: b.succeeded.length + b.failed.length,
        callsSucceeded: b.succeeded.length,
        callsFailed: b.failed.length,
  ...
  const crossCompetitionTransition = {
    reachedProviderSeason2024: seasonsObserved.includes("2024"),
  ...
  const aetPenAudit: Pe4I8AetPenAuditRow[] = [];
  for (const id of selection.selectedFixtureIds) {
    const meta = metaMap.get(id);
  ...
  put("batch-manifest.json", {
    stage2cVersion: PE4I8_STAGE2C_VERSION,
    acquiredAtUtc,
  ...
  put("provider-season-coverage.json", { providerSeasonCoverage });
...
export function pe4I8ArtifactDir(cwd = process.cwd()): string {
