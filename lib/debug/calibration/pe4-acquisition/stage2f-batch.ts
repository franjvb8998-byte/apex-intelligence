 ...
 * Same frozen queue / auth / hard max 150. Keeps competition coverage matrix
 * (observed-sample xG labels only — not model branches), provider season 2024
 * cumulative audit, and emphasizes cumulative calendar-2025 raw fixture audit
 * across all Stage-2 cached queue envelopes. No modeling.
 */
...
export const PE4I11_STAGE2F_VERSION =
  "pe4.stage2f_statistics_batch.v1" as const;

export const PE4I11_ARTIFACT_SUBDIR = "stage2f-i11" as const;

/** Authoritative post-PE-4I.10 cache state before Stage-2F. */
export const PE4I11_EXPECTED_INITIAL_CACHE_HITS = 751 as const;
export const PE4I11_EXPECTED_INITIAL_UNCACHED = 343 as const;

/** Coverage labels only — NOT model branches / capability gates. */
export type Pe4I11ObservedXgLabel =
  | "XG_CAPABLE_IN_OBSERVED_SAMPLE"
  | "XG_PARTIAL_IN_OBSERVED_SAMPLE"
  | "XG_UNAVAILABLE_IN_OBSERVED_SAMPLE";

export function classifyPe4I11ObservedXgLabel(
  bothShare: number,
): Pe4I11ObservedXgLabel {
...
function artifactRoot(cwd: string): string {
  return path.join(cwd, PE4I2_CACHE_ROOT_RELATIVE, PE4I11_ARTIFACT_SUBDIR);
}
...
export type Pe4I11AetPenAuditRow = {
  providerFixtureId: string;
  status: string;
  kickoffUtc: string;
...
export type Pe4I11StatusAuditRow = {
  providerFixtureId: string;
  status: string;
  competition: string | null;
...
export type Pe4I11CompetitionFieldCoverageRow = {
  competition: string;
  fixtureCount: number;
...
export type Pe4I11SeasonCoverageRow = {
...
export type Pe4I11CompetitionCoverageMatrixRow = {
  competition: string;
  fixtureCount: number;
  xgBothTeamFixtures: number;
  xgOneTeamFixtures: number;
...
function buildCompetitionFieldCoverage(
  ...
  envelopes: readonly Pe4I2RawStatisticsEnvelope[],
  metaMap: ReadonlyMap<string, Pe4I6FixtureMeta>,
): Pe4I11CompetitionFieldCoverageRow {
...
function buildCompetitionCoverageMatrixRow(
  ...
  envelopes: readonly Pe4I2RawStatisticsEnvelope[],
  metaMap: ReadonlyMap<string, Pe4I6FixtureMeta>,
): Pe4I11CompetitionCoverageMatrixRow {
  const xgLocal = measureXgCoverage({
    envelopes,
    metaByFixtureId: metaMap,
  });
  ...
  return {
    ...
    yellowCardsCoveragePercent: fieldPresentShare(envelopes, "yellow_cards"),
    redCardsCoveragePercent: fieldPresentShare(envelopes, "red_cards"),
    observedXgLabel: classifyPe4I11ObservedXgLabel(bothShare),
    observedXgLabelNote:
...
function buildProviderSeason2024Coverage(
  ...
  envelopes: readonly Pe4I2RawStatisticsEnvelope[],
  metaMap: ReadonlyMap<string, Pe4I6FixtureMeta>,
): Pe4I11ProviderSeason2024Coverage | null {
...
export type Pe4I11Stage2FReport = {
  phase: "PE4I11_STAGE2F_STATISTICS_BATCH";
  stage2fVersion: typeof PE4I11_STAGE2F_VERSION;
  queueDigest: string;
  queueSize: number;
  ...
  competitionSpecificCoverage: Pe4I11CompetitionFieldCoverageRow[];
  cumulativeCompetitionCoverage: Pe4I11CompetitionFieldCoverageRow[];
  competitionCoverageMatrix: Pe4I11CompetitionCoverageMatrixRow[];
  competitionCoverageMatrixNote: string;
  providerSeasonCoverage: Pe4I11SeasonCoverageRow[];
  providerSeason2024TransitionReached: boolean;
  providerSeason2024Coverage: Pe4I11ProviderSeason2024Coverage | null;
  ...
  statusAudit: Pe4I11StatusAuditRow[];
  aetPenAudit: Pe4I11AetPenAuditRow[];
...
export async function runPe4I11Stage2FStatisticsBatch(input: {
  transport: Pe4I2ProviderTransport;
  maxCalls: number;
  cwd?: string;
  acquiredAtUtc?: string;
}): Promise<Pe4I11Stage2FReport> {
  ...
  if (baseSelection.initialCachedCount !== PE4I11_EXPECTED_INITIAL_CACHE_HITS) {
    throw new Error(
      `STOP: expected exactly ${PE4I11_EXPECTED_INITIAL_CACHE_HITS} statistics cache hits before Stage-2F; got ${baseSelection.initialCachedCount}`,
    );
  }
  if (baseSelection.initialUncachedCount !== PE4I11_EXPECTED_INITIAL_UNCACHED) {
    throw new Error(
      `STOP: expected exactly ${PE4I11_EXPECTED_INITIAL_UNCACHED} uncached before Stage-2F; got ${baseSelection.initialUncachedCount}`,
    );
  }
  ...
  const unitResults: Pe4I11Stage2FReport["firstPass"]["unitResults"] = [];
  let newWrites = 0;
  ...
  const competitionCoverageMatrix: Pe4I11CompetitionCoverageMatrixRow[] = (() => {
    const byComp = new Map<
      string,
      { ids: string[]; envelopes: Pe4I2RawStatisticsEnvelope[] }
    >();
  ...
  const providerSeasonCoverage: Pe4I11SeasonCoverageRow[] = [...bySeason.entries()]
    .map(([season, b]) => {
      const xgLocal = measureXgCoverage({
  ...
  const statusAudit: Pe4I11StatusAuditRow[] = [];
  const aetPenAudit: Pe4I11AetPenAuditRow[] = [];
  ...
  put("batch-manifest.json", {
    stage2fVersion: PE4I11_STAGE2F_VERSION,
    acquiredAtUtc,
...
export function pe4I11ArtifactDir(cwd = process.cwd()): string {
