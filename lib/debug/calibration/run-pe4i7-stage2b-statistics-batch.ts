/**
 * PE-4I.7 — Stage-2B statistics batch CLI (max 150 new provider calls).
 *
 *   npx tsx lib/debug/calibration/run-pe4i7-stage2b-statistics-batch.ts \
...
import {
  pe4I7ArtifactDir,
  runPe4I7Stage2BStatisticsBatch,
} from "@/lib/debug/calibration/pe4-acquisition/stage2b-batch";
import {
...
async function main(): Promise<void> {
  ...
  if (preview.initialCachedCount < 151) {
    throw new Error(
      `STOP: expected >=151 cache hits before Stage-2B; got ${preview.initialCachedCount}`,
    );
  }
  if (preview.initialUncachedCount > 943) {
    throw new Error(
      `STOP: uncached ${preview.initialUncachedCount} exceeds expected 943`,
    );
  }
  ...
  const report = await runPe4I7Stage2BStatisticsBatch({
    transport,
    maxCalls,
  });

  const reportPath = path.join(pe4I7ArtifactDir(), "stage2b-report.json");
  writeFileSync(reportPath, JSON.stringify(report, null, 2) + "\n", "utf8");
  console.log(JSON.stringify(report, null, 2));
  console.log(
    JSON.stringify(
      {
        stage2bReportPath: path
          .relative(process.cwd(), reportPath)
          .replace(/\\/g, "/"),
        artifactDir: path
          .relative(process.cwd(), pe4I7ArtifactDir())
          .replace(/\\/g, "/"),
      },
      null,
      2,
...
const invoked =
  process.argv[1]
    ?.replace(/\\/g, "/")
    .endsWith("run-pe4i7-stage2b-statistics-batch.ts") === true;
