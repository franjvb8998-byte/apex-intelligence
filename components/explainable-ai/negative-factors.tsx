...
export function NegativeFactors({ factors }: NegativeFactorsProps) {
  ...
  return (
    <Card padding="sm">
      <CardHeader
        className="mb-3"
        title={t("negativeFactors")}
        description={t("negativeFactorsDescription")}
      />
        ...
        {factors.map((factor) => (
          <li key={factor.id}>
            <div className="flex items-baseline justify-between gap-2">
              ...
              <span className="shrink-0 font-mono text-[11px] tabular-nums text-[var(--apex-warning)]">
